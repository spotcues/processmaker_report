...
class ReportsEmailEvent implements ShouldQueue
{
    ...
    public function handle(Reports $reportsObj)
    {
        try {
            ...
            $this->reports_logger->clog($this->reports_logger, ' :: starting job processing for excel generation request :: '.$this->payload);
            ...
            $pro_title = trim($job_payload['pro_title']);
            ...
            unset($job_payload['pro_title']);
            unset($job_payload['workspace_name']);
            
            $report_excel_link = $reportsObj->getFormDetailsVersion2ExcelGenerate(
                $job_payload['start'],
                $job_payload['limit'],
                ...
                $job_payload['status'],
                $job_payload['completion'],
                $job_payload['current_step'],
            ...
            $mail_text = "Hello,".PHP_EOL.PHP_EOL."Please click on the link below to download the ".$pro_title. " process details report from the ".$workspace_name." spot";
            ...
            $mail_payload = [
                'to' => $send_to_email,
                'cc' => $cc_to_email,
                'subject' => 'Groupe.io - Download the '. $pro_title .' process details report',
                'text' => $mail_text,
            ];
            ...
            $this->reports_logger->clog($this->reports_logger, 'Process Reports Excel Generation Failed due to : '.PHP_EOL.$exception->getMessage().PHP_EOL.'Payload : '.$this->payload);
            $this->fail($exception);
        }
    }
    ...
    public function failed(\Exception $exception)
    {
        ...
        $notification_payload = [
            ...
            'subject' => 'Process Reports Excel Generation Failure',
            'text' => 'Process Reports Excel Generation Failed due to : '.PHP_EOL.$exception->getMessage().PHP_EOL.PHP_EOL.PHP_EOL.'Payload : '.$this->payload
        ];
