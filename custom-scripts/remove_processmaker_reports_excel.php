<?php
/**
 * php script to remove reports excel files for all workspaces.
 * Usage : sudo php remove_processmaker_reports_excel.php
 * @var string
 */
...
do {
	echo "Enter number of days (integer number like 1,2 etc. ). Processmaker reports excel files older than this value would get deleted. [DEFAULT : 1]. To exit press X ".PHP_EOL;
	$handle = fopen ("php://stdin","r");
	...
	if($number_of_days < 1){
		echo "Value less than 1 was given. This would delete yesterday's reports excel files also and only preserve files which got generated today. Do you want to proceed [Y/N]. To exit press X ".PHP_EOL;
		$handle = fopen ("php://stdin","r");
		$confirm_choice = fgets($handle);
		fclose($handle);
...
if(trim($user_input_pm_root) == null || trim($user_input_pm_root) == '') {
	...
	if(!file_exists($dirname) || !is_dir($dirname)) {
		echo "Processmaker install directory info is incorrect. If you think path is correct, then permissions issue might be there. Run the script as root or use sudo. Enter correct path. Aborting !!!!";
		exit(0);
	}
	...
	foreach ($workspaces as $workspace_path) {
		...
		$workspace_reports_excel_path = "$workspace_path/public/reports";

		if(file_exists($workspace_reports_excel_path)  && is_dir($workspace_reports_excel_path)) {
			echo "reports excel directory exists for workspace - $workspace_name. checking for reports excel files now - ".PHP_EOL;
			$scan_arr = scandir($workspace_reports_excel_path);
			$files_arr = array_diff($scan_arr, array('.','..') );

			foreach ($files_arr as $file) {

				$file_path = "$workspace_reports_excel_path/".$file;
				$file_ext = pathinfo($file_path, PATHINFO_EXTENSION);
				$file_name = pathinfo($file_path, PATHINFO_FILENAME);
				$base_file_name = pathinfo($file_path, PATHINFO_BASENAME);

				if ($file_ext === "xlsx" && strpos($file_name, 'Process_Details-') !== false) {
					echo 'Found excel file : ' . $base_file_name . PHP_EOL;
					$files_found++;
					...
					if ( (time() - $file_creation_date ) > USER_INPUT_NUMBER_OF_DAYS*24*3600){
						...
						try{
							unlink($file_path);
							echo "Deleted reports excel file $base_file_name from $workspace_name successfully.".PHP_EOL;
							$files_deleted++;
						} catch(\Exception $e)
						{
							echo "Failed to deleted $base_file_name for $workspace_name. ".json_encode($e->getMessage()).". Continuing with next reports excel file.".PHP_EOL;
						}
					} else {
			...
			echo "No reports excel directory found for workspace - $workspace. Continuing !!!".PHP_EOL;
		}
	}
}
echo PHP_EOL;
echo "Total reports excel files found per given criteria were : $files_found".PHP_EOL;
echo "Total reports excel files deleted per given criteria were : $files_deleted".PHP_EOL;

echo "Finished removing old reports excel files from workspaces, exiting...!!!!". PHP_EOL;die;
?>
